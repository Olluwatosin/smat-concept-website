...
import {
  Database, Check, ArrowRight, MessageCircle,
  Search, Cloud, FileText, ShieldCheck, RefreshCw, Code,
  Clock, Lock, TrendingUp,
} from "lucide-react";
...
const features = [
  "High-volume document scanning (up to 10,000 pages/day)",
  "OCR text extraction with 99%+ accuracy",
  "Intelligent document classification & tagging",
  "Secure cloud storage with redundant backups",
  "Full-text search across your entire archive",
  "Compliance-ready audit trail & version history",
  "Multi-format conversion (PDF, DOCX, TIFF, searchable PDF)",
  "RESTful API access for system integration",
];

const steps = [
  { num: "01", title: "Document Audit", desc: "We assess your paper archive volume, document types, condition, and compliance requirements to scope the project." },
  { num: "02", title: "Scanning & Capture", desc: "High-speed industrial scanners capture every page at archival quality. Fragile documents handled with professional care." },
  { num: "03", title: "OCR & Classification", desc: "AI-powered OCR extracts searchable text. Documents are automatically classified, tagged, and indexed." },
  { num: "04", title: "Cloud Delivery", desc: "Your digital archive is uploaded to a secure portal, integrated with your existing systems, and handed over with training." },
];

const benefits = [
  { icon: Clock, title: "Find Any Document in Seconds", desc: "Full-text search across thousands of records instantly — no more digging through filing cabinets." },
  { icon: Lock, title: "Compliance & Security", desc: "Role-based access, audit trails, and encryption ensure your sensitive records stay protected and auditable." },
  { icon: TrendingUp, title: "Reclaim Office Space", desc: "Eliminate physical filing rooms. A full floor of paper archives fits on a single cloud server." },
];

export default function DocumentDigitalizationPage() {
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#080B16", color: "#F9FAFB" }}>
      ...
      <section className="relative pt-20 lg:pt-24 pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center py-20">
          <motion.div {...fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6 glass-card" style={{ border: "1px solid rgba(212,175,55,0.3)" }}>
            ...
            <span className="text-sm font-medium" style={{ color: "#D4AF37" }}>Document Digitalization</span>
          </motion.div>
          <motion.h1 {...fadeUp} transition={{ delay: 0.1, duration: 0.55 }} className="text-2xl sm:text-4xl md:text-6xl font-bold mb-4 leading-tight">
            <span className="text-gold-gradient">From Paper Archives</span>
            <br /><span className="text-white">to Searchable Intelligence</span>
          </motion.h1>
          <motion.p {...fadeUp} transition={{ delay: 0.2, duration: 0.55 }} className="text-lg text-gray-400 max-w-2xl mx-auto mb-8">
            We convert your physical document archives into structured, searchable, cloud-hosted digital assets — ready for instant retrieval and compliance.
          </motion.p>
      ...
      <section className="section-divider py-16 max-w-5xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 text-gray-300 leading-relaxed">
          <motion.p {...fadeUp} transition={{ delay: 0.1 }}>
            SMAT Concept's Document Digitalization service is an end-to-end transformation of your paper-based records into a structured digital archive. Using enterprise-grade scanners, AI-powered OCR, and intelligent classification engines, we process everything from hand-written field notes to thick legal dossiers — at scale, with precision.
          </motion.p>
          <motion.p {...fadeUp} transition={{ delay: 0.2 }}>
            Built for banks, hospitals, law firms, government ministries, logistics companies, and any organisation drowning in paperwork. We also handle legacy digital archives — disorganised scanned PDFs, image files, and mixed-format repositories — transforming them into a fully searchable, properly tagged knowledge base.
          </motion.p>
        </div>
      </section>
      ...
      <section className="section-divider py-16 max-w-5xl mx-auto px-6 text-center">
        <motion.h2 {...fadeUp} className="text-3xl font-bold mb-4 text-white">Ready to Go Paperless?</motion.h2>
        <motion.p {...fadeUp} transition={{ delay: 0.1 }} className="text-gray-400 mb-8 max-w-xl mx-auto">
          Let's assess your archive and give you a clear digitalization roadmap — timeline, cost, and expected ROI included.
        </motion.p>
